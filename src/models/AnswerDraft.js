...
class AnswerDraft {
  static async update(applicationId) {
    const connection = await pool.getConnection();
    try {
      await connection.beginTransaction();

      // Call the stored procedure to update the answer draft flag
      await connection.execute('SET @error_code = 0');
      await connection.execute(
        'CALL UpdateAnswerDraftFlag(?, @error_code)',
        [applicationId]
      );

      // Get the error code from the stored procedure
      const [[{ error_code }]] = await connection.execute('SELECT @error_code as error_code');

      if (error_code !== 0) {
        throw new Error('Failed to update answer draft flag');
      }
  ...
  static async create(finalAnswerData) {
    ...
    try {
      ...
      // Update answer draft flag after successful creation
      await connection.execute('SET @error_code = 0');
      await connection.execute(
        'CALL UpdateAnswerDraftFlag(?, @error_code)',
        [application_id]
      ...
      const [[{ error_code: draftError }]] = await connection.execute('SELECT @error_code as error_code');
      if (draftError !== 0) {
        throw new Error('Failed to update answer draft flag');
      }
