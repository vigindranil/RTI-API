...
class Report {
  ...
  static async getApplicationCountByArea(formattedStartDate, formattedEndDate, userId) {
    const [results] = await pool.execute('CALL sp_get_application_count_by_area(?, ?, ?)', [
      formattedStartDate,
      formattedEndDate,
      userId
    ]);
    
    const areaData = results[0];
    const totalApplications = areaData.reduce((sum, item) => sum + item.application_count, 0);

    return areaData.map(row => ({
      area: row.area,
      application_count: row.application_count,
      percentage: ((row.application_count / totalApplications) * 100).toFixed(2)
    }));
  }
